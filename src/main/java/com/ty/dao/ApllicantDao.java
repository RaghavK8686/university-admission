...
public class ApllicantDao {
	...
	public List<Applicant> filterByPer(double percent)
	{

		String sql="select s from Applicant s where s.puc>=?1";
		Query query=entityManager.createQuery(sql);
		query.setParameter(1, percent);
		List<Applicant> applicant1=query.getResultList();
		if(applicant1!=null)
		{
			return applicant1;
		}
	...
	{
		String sql="select s from Applicant s ";
		Query query=entityManager.createQuery(sql);
		List<Applicant> applicant1=query.getResultList();
		if(applicant1!=null)
		{
